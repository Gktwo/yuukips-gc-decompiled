...
@Entity
/* loaded from: grasscutter.jar:emu/grasscutter/game/managers/forging/ActiveForgeData.class */
public class ActiveForgeData {
    ...
    public int getFinishedCount(int currentTime) {
        return Math.min((currentTime - this.startTime) / this.forgeTime, this.count);
    }

    public int getUnfinishedCount(int currentTime) {
        return this.count - getFinishedCount(currentTime);
    }

    public int getNextFinishTimestamp(int currentTime) {
        if (currentTime >= getTotalFinishTimestamp()) {
            return getTotalFinishTimestamp();
        }
        return (getFinishedCount(currentTime) * this.forgeTime) + this.forgeTime + this.startTime;
    }
    ...
    public boolean updateChanged(int currentTime) {
        int currentUnfinished = getUnfinishedCount(currentTime);
