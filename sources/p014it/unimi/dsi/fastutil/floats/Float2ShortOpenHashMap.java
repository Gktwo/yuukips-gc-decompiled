...
public class Float2ShortOpenHashMap extends AbstractFloat2ShortMap implements Serializable, Cloneable, Hash {
    ...
    /* renamed from: n */
    protected transient int f1915n;
    protected transient int maxFill;
    protected final transient int minN;
    protected int size;

    /* renamed from: f */
    protected final float f1916f;
    ...
    public Float2ShortOpenHashMap(int expected, float f) {
        if (f <= 0.0f || f >= 1.0f) {
            ...
            this.f1915n = arraySize;
            this.minN = arraySize;
            this.mask = this.f1915n - 1;
            this.maxFill = HashCommon.maxFill(this.f1915n, f);
            this.key = new float[this.f1915n + 1];
            this.value = new short[this.f1915n + 1];
        }
    }
    ...
    private void ensureCapacity(int capacity) {
        int needed = HashCommon.arraySize(capacity, this.f1916f);
        if (needed > this.f1915n) {
            rehash(needed);
        }
    }

    private void tryCapacity(long capacity) {
        int needed = (int) Math.min((long) FileSize.GB_COEFFICIENT, Math.max(2L, HashCommon.nextPowerOfTwo((long) Math.ceil((double) (((float) capacity) / this.f1916f)))));
        if (needed > this.f1915n) {
            rehash(needed);
        }
    }
    ...
    public short removeEntry(int pos) {
        ...
        this.size--;
        shiftKeys(pos);
        if (this.f1915n > this.minN && this.size < this.maxFill / 4 && this.f1915n > 16) {
            rehash(this.f1915n / 2);
        }
        return oldValue;
    }
    ...
    public short removeNullEntry() {
        ...
        short oldValue = this.value[this.f1915n];
        this.size--;
        if (this.f1915n > this.minN && this.size < this.maxFill / 4 && this.f1915n > 16) {
            rehash(this.f1915n / 2);
        }
    ...
    private int find(float k) {
        ...
        if (Float.floatToIntBits(k) == 0) {
            return this.containsNullKey ? this.f1915n : -(this.f1915n + 1);
        }
        float[] key = this.key;
    ...
    private void insert(int pos, float k, short v) {
        ...
        this.size = i + 1;
        if (i >= this.maxFill) {
            rehash(HashCommon.arraySize(this.size + 1, this.f1916f));
        }
    }
    ...
    public short addTo(float k, short incr) {
        ...
        if (Float.floatToIntBits(k) != 0) {
            ...
            return addToValue(this.f1915n, incr);
        } else {
            pos = this.f1915n;
            this.containsNullKey = true;
        }
    ...
    public short get(float k) {
        float curr;
        if (Float.floatToIntBits(k) == 0) {
            return this.containsNullKey ? this.value[this.f1915n] : this.defRetValue;
        }
        float[] key = this.key;
        int mix = HashCommon.mix(HashCommon.float2int(k)) & this.mask;
        int pos = mix;
    ...
    public boolean containsValue(short v) {
        ...
        float[] key = this.key;
        if (this.containsNullKey && value[this.f1915n] == v) {
            return true;
        }
        int i = this.f1915n;
        while (true) {
    ...
    public short getOrDefault(float k, short defaultValue) {
        float curr;
        if (Float.floatToIntBits(k) == 0) {
            return this.containsNullKey ? this.value[this.f1915n] : defaultValue;
        }
        float[] key = this.key;
        int mix = HashCommon.mix(HashCommon.float2int(k)) & this.mask;
        int pos = mix;
    ...
    public boolean remove(float k, short v) {
        ...
        } else if (!this.containsNullKey || v != this.value[this.f1915n]) {
            return false;
        } else {
            removeNullEntry();
            return true;
    ...
    private abstract class MapIterator<ConsumerType> {
        ...
        private MapIterator() {
            this.pos = Float2ShortOpenHashMap.this.f1915n;
            this.last = -1;
            this.f1917c = Float2ShortOpenHashMap.this.size;
        ...
        public int nextEntry() {
            ...
            if (this.mustReturnNullKey) {
                ...
                int i = Float2ShortOpenHashMap.this.f1915n;
                this.last = i;
                return i;
            }
        ...
        public void remove() {
            ...
            if (this.last == Float2ShortOpenHashMap.this.f1915n) {
    ...
    private abstract class MapSpliterator<ConsumerType, SplitType extends MapSpliterator<ConsumerType, SplitType>> {
        ...
        MapSpliterator() {
            this.pos = 0;
            this.max = Float2ShortOpenHashMap.this.f1915n;
            this.f1918c = 0;
        ...
        public boolean tryAdvance(ConsumerType action) {
            if (this.mustReturnNull) {
                ...
                this.f1918c++;
                acceptOnIndex(action, Float2ShortOpenHashMap.this.f1915n);
                return true;
            }
            float[] key = Float2ShortOpenHashMap.this.key;
        ...
        public long estimateSize() {
            if (!this.hasSplit) {
                return (long) (Float2ShortOpenHashMap.this.size - this.f1918c);
            }
            return Math.min((long) (Float2ShortOpenHashMap.this.size - this.f1918c), ((long) ((((double) Float2ShortOpenHashMap.this.realSize()) / ((double) Float2ShortOpenHashMap.this.f1915n)) * ((double) (this.max - this.pos)))) + ((long) (this.mustReturnNull ? 1 : 0)));
        }
    ...
    public final class MapEntrySet extends AbstractObjectSet<Float2ShortMap.Entry> implements Float2ShortMap.FastEntrySet {
        ...
        public boolean contains(Object o) {
            ...
            if (Float.floatToIntBits(k) == 0) {
                return Float2ShortOpenHashMap.this.containsNullKey && Float2ShortOpenHashMap.this.value[Float2ShortOpenHashMap.this.f1915n] == v;
            }
            float[] key = Float2ShortOpenHashMap.this.key;
        ...
        public boolean remove(Object o) {
            ...
            } else if (!Float2ShortOpenHashMap.this.containsNullKey || Float2ShortOpenHashMap.this.value[Float2ShortOpenHashMap.this.f1915n] != v) {
                return false;
            } else {
        ...
        public void forEach(Consumer<? super Float2ShortMap.Entry> consumer) {
            if (Float2ShortOpenHashMap.this.containsNullKey) {
                consumer.accept(new AbstractFloat2ShortMap.BasicEntry(Float2ShortOpenHashMap.this.key[Float2ShortOpenHashMap.this.f1915n], Float2ShortOpenHashMap.this.value[Float2ShortOpenHashMap.this.f1915n]));
            }
            int pos = Float2ShortOpenHashMap.this.f1915n;
            while (true) {
                pos--;
                if (pos == 0) {
                    return;
        ...
        public void fastForEach(Consumer<? super Float2ShortMap.Entry> consumer) {
            ...
            if (Float2ShortOpenHashMap.this.containsNullKey) {
                entry.key = Float2ShortOpenHashMap.this.key[Float2ShortOpenHashMap.this.f1915n];
                entry.value = Float2ShortOpenHashMap.this.value[Float2ShortOpenHashMap.this.f1915n];
    ...
    public final class KeySet extends AbstractFloatSet {
        ...
        public void forEach(FloatConsumer consumer) {
            if (Float2ShortOpenHashMap.this.containsNullKey) {
                consumer.accept(Float2ShortOpenHashMap.this.key[Float2ShortOpenHashMap.this.f1915n]);
            }
            int pos = Float2ShortOpenHashMap.this.f1915n;
            while (true) {
                pos--;
        ...
        if (this.values == null) {
            this.values = new AbstractShortCollection() { // from class: it.unimi.dsi.fastutil.floats.Float2ShortOpenHashMap.1
                ...
                public void forEach(ShortConsumer consumer) {
                    if (Float2ShortOpenHashMap.this.containsNullKey) {
                        consumer.accept(Float2ShortOpenHashMap.this.value[Float2ShortOpenHashMap.this.f1915n]);
                    }
                    int pos = Float2ShortOpenHashMap.this.f1915n;
                    while (true) {
                        pos--;
    ...
    public boolean trim(int n) {
        int l = HashCommon.nextPowerOfTwo((int) Math.ceil((double) (((float) n) / this.f1916f)));
        if (l >= this.f1915n || this.size > HashCommon.maxFill(l, this.f1916f)) {
            return true;
        }
        try {
            rehash(l);
    ...
    protected void rehash(int newN) {
        ...
        while (true) {
            ...
            if (j != 0) {
                ...
                newValue[newN] = value[this.f1915n];
                this.f1915n = newN;
                this.mask = mask;
                this.maxFill = HashCommon.maxFill(this.f1915n, this.f1916f);
                this.key = newKey;
                this.value = newValue;
                return;
            }
    ...
    private void readObject(ObjectInputStream s) throws IOException, ClassNotFoundException {
        int pos;
        s.defaultReadObject();
        this.f1915n = HashCommon.arraySize(this.size, this.f1916f);
        this.maxFill = HashCommon.maxFill(this.f1915n, this.f1916f);
        this.mask = this.f1915n - 1;
        float[] key = new float[this.f1915n + 1];
        this.key = key;
        short[] value = new short[this.f1915n + 1];
        this.value = value;
        int i = this.size;
        while (true) {
            i--;
