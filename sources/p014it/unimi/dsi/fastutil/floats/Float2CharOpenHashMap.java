...
public class Float2CharOpenHashMap extends AbstractFloat2CharMap implements Serializable, Cloneable, Hash {
    ...
    /* renamed from: n */
    protected transient int f1795n;
    protected transient int maxFill;
    protected final transient int minN;
    protected int size;

    /* renamed from: f */
    protected final float f1796f;
    ...
    public Float2CharOpenHashMap(int expected, float f) {
        if (f <= 0.0f || f >= 1.0f) {
            ...
            this.f1795n = arraySize;
            this.minN = arraySize;
            this.mask = this.f1795n - 1;
            this.maxFill = HashCommon.maxFill(this.f1795n, f);
            this.key = new float[this.f1795n + 1];
            this.value = new char[this.f1795n + 1];
        }
    }
    ...
    private void ensureCapacity(int capacity) {
        int needed = HashCommon.arraySize(capacity, this.f1796f);
        if (needed > this.f1795n) {
            rehash(needed);
        }
    }

    private void tryCapacity(long capacity) {
        int needed = (int) Math.min((long) FileSize.GB_COEFFICIENT, Math.max(2L, HashCommon.nextPowerOfTwo((long) Math.ceil((double) (((float) capacity) / this.f1796f)))));
        if (needed > this.f1795n) {
            rehash(needed);
        }
    }
    ...
    public char removeEntry(int pos) {
        ...
        this.size--;
        shiftKeys(pos);
        if (this.f1795n > this.minN && this.size < this.maxFill / 4 && this.f1795n > 16) {
            rehash(this.f1795n / 2);
        }
        return oldValue;
    }
    ...
    public char removeNullEntry() {
        ...
        char oldValue = this.value[this.f1795n];
        this.size--;
        if (this.f1795n > this.minN && this.size < this.maxFill / 4 && this.f1795n > 16) {
            rehash(this.f1795n / 2);
        }
    ...
    private int find(float k) {
        ...
        if (Float.floatToIntBits(k) == 0) {
            return this.containsNullKey ? this.f1795n : -(this.f1795n + 1);
        }
        float[] key = this.key;
    ...
    private void insert(int pos, float k, char v) {
        ...
        this.size = i + 1;
        if (i >= this.maxFill) {
            rehash(HashCommon.arraySize(this.size + 1, this.f1796f));
        }
    }
    ...
    public char addTo(float k, char incr) {
        ...
        if (Float.floatToIntBits(k) != 0) {
            ...
            return addToValue(this.f1795n, incr);
        } else {
            pos = this.f1795n;
            this.containsNullKey = true;
        }
    ...
    public char get(float k) {
        float curr;
        if (Float.floatToIntBits(k) == 0) {
            return this.containsNullKey ? this.value[this.f1795n] : this.defRetValue;
        }
        float[] key = this.key;
        int mix = HashCommon.mix(HashCommon.float2int(k)) & this.mask;
        int pos = mix;
    ...
    public boolean containsValue(char v) {
        ...
        float[] key = this.key;
        if (this.containsNullKey && value[this.f1795n] == v) {
            return true;
        }
        int i = this.f1795n;
        while (true) {
    ...
    public char getOrDefault(float k, char defaultValue) {
        float curr;
        if (Float.floatToIntBits(k) == 0) {
            return this.containsNullKey ? this.value[this.f1795n] : defaultValue;
        }
        float[] key = this.key;
        int mix = HashCommon.mix(HashCommon.float2int(k)) & this.mask;
        int pos = mix;
    ...
    public boolean remove(float k, char v) {
        ...
        } else if (!this.containsNullKey || v != this.value[this.f1795n]) {
            return false;
        } else {
            removeNullEntry();
            return true;
    ...
    private abstract class MapIterator<ConsumerType> {
        ...
        private MapIterator() {
            this.pos = Float2CharOpenHashMap.this.f1795n;
            this.last = -1;
            this.f1797c = Float2CharOpenHashMap.this.size;
        ...
        public int nextEntry() {
            ...
            if (this.mustReturnNullKey) {
                ...
                int i = Float2CharOpenHashMap.this.f1795n;
                this.last = i;
                return i;
            }
        ...
        public void remove() {
            ...
            if (this.last == Float2CharOpenHashMap.this.f1795n) {
    ...
    private abstract class MapSpliterator<ConsumerType, SplitType extends MapSpliterator<ConsumerType, SplitType>> {
        ...
        MapSpliterator() {
            this.pos = 0;
            this.max = Float2CharOpenHashMap.this.f1795n;
            this.f1798c = 0;
        ...
        public boolean tryAdvance(ConsumerType action) {
            if (this.mustReturnNull) {
                ...
                this.f1798c++;
                acceptOnIndex(action, Float2CharOpenHashMap.this.f1795n);
                return true;
            }
            float[] key = Float2CharOpenHashMap.this.key;
        ...
        public long estimateSize() {
            if (!this.hasSplit) {
                return (long) (Float2CharOpenHashMap.this.size - this.f1798c);
            }
            return Math.min((long) (Float2CharOpenHashMap.this.size - this.f1798c), ((long) ((((double) Float2CharOpenHashMap.this.realSize()) / ((double) Float2CharOpenHashMap.this.f1795n)) * ((double) (this.max - this.pos)))) + ((long) (this.mustReturnNull ? 1 : 0)));
        }
    ...
    public final class MapEntrySet extends AbstractObjectSet<Float2CharMap.Entry> implements Float2CharMap.FastEntrySet {
        ...
        public boolean contains(Object o) {
            ...
            if (Float.floatToIntBits(k) == 0) {
                return Float2CharOpenHashMap.this.containsNullKey && Float2CharOpenHashMap.this.value[Float2CharOpenHashMap.this.f1795n] == v;
            }
            float[] key = Float2CharOpenHashMap.this.key;
        ...
        public boolean remove(Object o) {
            ...
            } else if (!Float2CharOpenHashMap.this.containsNullKey || Float2CharOpenHashMap.this.value[Float2CharOpenHashMap.this.f1795n] != v) {
                return false;
            } else {
        ...
        public void forEach(Consumer<? super Float2CharMap.Entry> consumer) {
            if (Float2CharOpenHashMap.this.containsNullKey) {
                consumer.accept(new AbstractFloat2CharMap.BasicEntry(Float2CharOpenHashMap.this.key[Float2CharOpenHashMap.this.f1795n], Float2CharOpenHashMap.this.value[Float2CharOpenHashMap.this.f1795n]));
            }
            int pos = Float2CharOpenHashMap.this.f1795n;
            while (true) {
                pos--;
                if (pos == 0) {
                    return;
        ...
        public void fastForEach(Consumer<? super Float2CharMap.Entry> consumer) {
            ...
            if (Float2CharOpenHashMap.this.containsNullKey) {
                entry.key = Float2CharOpenHashMap.this.key[Float2CharOpenHashMap.this.f1795n];
                entry.value = Float2CharOpenHashMap.this.value[Float2CharOpenHashMap.this.f1795n];
    ...
    public final class KeySet extends AbstractFloatSet {
        ...
        public void forEach(FloatConsumer consumer) {
            if (Float2CharOpenHashMap.this.containsNullKey) {
                consumer.accept(Float2CharOpenHashMap.this.key[Float2CharOpenHashMap.this.f1795n]);
            }
            int pos = Float2CharOpenHashMap.this.f1795n;
            while (true) {
                pos--;
        ...
        if (this.values == null) {
            this.values = new AbstractCharCollection() { // from class: it.unimi.dsi.fastutil.floats.Float2CharOpenHashMap.1
                ...
                public void forEach(CharConsumer consumer) {
                    if (Float2CharOpenHashMap.this.containsNullKey) {
                        consumer.accept(Float2CharOpenHashMap.this.value[Float2CharOpenHashMap.this.f1795n]);
                    }
                    int pos = Float2CharOpenHashMap.this.f1795n;
                    while (true) {
                        pos--;
    ...
    public boolean trim(int n) {
        int l = HashCommon.nextPowerOfTwo((int) Math.ceil((double) (((float) n) / this.f1796f)));
        if (l >= this.f1795n || this.size > HashCommon.maxFill(l, this.f1796f)) {
            return true;
        }
        try {
            rehash(l);
    ...
    protected void rehash(int newN) {
        ...
        while (true) {
            ...
            if (j != 0) {
                ...
                newValue[newN] = value[this.f1795n];
                this.f1795n = newN;
                this.mask = mask;
                this.maxFill = HashCommon.maxFill(this.f1795n, this.f1796f);
                this.key = newKey;
                this.value = newValue;
                return;
            }
    ...
    private void readObject(ObjectInputStream s) throws IOException, ClassNotFoundException {
        int pos;
        s.defaultReadObject();
        this.f1795n = HashCommon.arraySize(this.size, this.f1796f);
        this.maxFill = HashCommon.maxFill(this.f1795n, this.f1796f);
        this.mask = this.f1795n - 1;
        float[] key = new float[this.f1795n + 1];
        this.key = key;
        char[] value = new char[this.f1795n + 1];
        this.value = value;
        int i = this.size;
        while (true) {
            i--;
