...
/* loaded from: grasscutter.jar:javassist/bytecode/Bytecode.class */
public class Bytecode extends ByteVector implements Cloneable, Opcode {
    ...
    public int read32bit(int offset) {
        return (read16bit(offset) << 16) + (read16bit(offset + 2) & 65535);
    }
    ...
    private void addPutfield0(CtClass target, String classname, String name, String desc) {
        add(181);
        addIndex(this.constPool.addFieldrefInfo(classname == null ? this.constPool.addClassInfo(target) : this.constPool.addClassInfo(classname), name, desc));
    ...
    private void addPutstatic0(CtClass target, String classname, String fieldName, String desc) {
        add(179);
        addIndex(this.constPool.addFieldrefInfo(classname == null ? this.constPool.addClassInfo(target) : this.constPool.addClassInfo(classname), fieldName, desc));
        growStack(-Descriptor.dataSize(desc));
    }
