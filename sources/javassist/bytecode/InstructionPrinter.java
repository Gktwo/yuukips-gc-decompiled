...
/* loaded from: grasscutter.jar:javassist/bytecode/InstructionPrinter.class */
public class InstructionPrinter implements Opcode {
    ...
    private static String classInfo(ConstPool pool, int index) {
        return LineReaderImpl.DEFAULT_COMMENT_BEGIN + index + " = Class " + pool.getClassInfo(index);
    }

    private static String interfaceMethodInfo(ConstPool pool, int index) {
        return LineReaderImpl.DEFAULT_COMMENT_BEGIN + index + " = Method " + pool.getInterfaceMethodrefClassName(index) + Mapper.IGNORED_FIELDNAME + pool.getInterfaceMethodrefName(index) + "(" + pool.getInterfaceMethodrefType(index) + ")";
    }

    private static String methodInfo(ConstPool pool, int index) {
        return LineReaderImpl.DEFAULT_COMMENT_BEGIN + index + " = Method " + pool.getMethodrefClassName(index) + Mapper.IGNORED_FIELDNAME + pool.getMethodrefName(index) + "(" + pool.getMethodrefType(index) + ")";
    }

    private static String fieldInfo(ConstPool pool, int index) {
        return LineReaderImpl.DEFAULT_COMMENT_BEGIN + index + " = Field " + pool.getFieldrefClassName(index) + Mapper.IGNORED_FIELDNAME + pool.getFieldrefName(index) + "(" + pool.getFieldrefType(index) + ")";
    }
    ...
    private static String ldc(ConstPool pool, int index) {
        ...
        switch (tag) {
            case 3:
                return LineReaderImpl.DEFAULT_COMMENT_BEGIN + index + " = int " + pool.getIntegerInfo(index);
            case 4:
                return LineReaderImpl.DEFAULT_COMMENT_BEGIN + index + " = float " + pool.getFloatInfo(index);
            case 5:
                return LineReaderImpl.DEFAULT_COMMENT_BEGIN + index + " = long " + pool.getLongInfo(index);
            case 6:
                return LineReaderImpl.DEFAULT_COMMENT_BEGIN + index + " = double " + pool.getDoubleInfo(index);
            case 7:
                return classInfo(pool, index);
            case 8:
                return LineReaderImpl.DEFAULT_COMMENT_BEGIN + index + " = \"" + pool.getStringInfo(index) + "\"";
            default:
