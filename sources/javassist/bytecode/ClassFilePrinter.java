...
/* loaded from: grasscutter.jar:javassist/bytecode/ClassFilePrinter.class */
public class ClassFilePrinter {
    ...
    public static void print(ClassFile cf, PrintWriter out) {
        ...
        for (FieldInfo finfo : cf.getFields()) {
            out.println(Modifier.toString(AccessFlag.toModifier(finfo.getAccessFlags())) + " " + finfo.getName() + SyslogAppender.DEFAULT_STACKTRACE_PATTERN + finfo.getDescriptor());
            printAttributes(finfo.getAttributes(), out, 'f');
        }
        out.println();
        for (MethodInfo minfo : cf.getMethods()) {
            out.println(Modifier.toString(AccessFlag.toModifier(minfo.getAccessFlags())) + " " + minfo.getName() + SyslogAppender.DEFAULT_STACKTRACE_PATTERN + minfo.getDescriptor());
