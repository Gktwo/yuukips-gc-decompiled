...
/* loaded from: grasscutter.jar:kcp/highway/erasure/fecNative/FecEncode.class */
public class FecEncode implements IFecEncode {
    ...
    public ByteBuf[] encode(ByteBuf byteBuf) {
        ...
        for (int i = 0; i < parityShards; i++) {
            ...
            shards[i + dataShards] = parityByte.memoryAddress() + ((long) payloadOffset);
        }
        for (int i2 = 0; i2 < dataShards; i2++) {
            ByteBuf shard = shardCache[i2];
            shards[i2] = shard.memoryAddress() + ((long) payloadOffset);
