...
/* loaded from: grasscutter.jar:net/bytebuddy/utility/visitor/StackAwareMethodVisitor.class */
public class StackAwareMethodVisitor extends MethodVisitor {
    ...
    private void adjustStack(int delta, int offset) {
        if (delta > 2) {
            ...
            this.current.add(position, StackSize.m123of(delta));
        } else if (offset != 0) {
    ...
    public int drainStack(int store, int load, StackSize size) {
        ...
        visitVarInsn(store, this.freeIndex);
        if (difference == 1) {
            visitInsn(87);
        ...
        visitVarInsn(load, this.freeIndex);
    ...
    public void visitInvokeDynamicInsn(String name, String descriptor, Handle bootstrap, Object... bootstrapArguments) {
        ...
        adjustStack(baseline & 3);
        visitInvokeDynamicInsn(name, descriptor, bootstrap, bootstrapArguments);
    }
    ...
    public void visitJumpInsn(int opcode, Label label) {
        adjustStack(SIZE_CHANGE[opcode]);
        this.sizes.put(label, new ArrayList(opcode == 168 ? CompoundList.m81of(this.current, StackSize.SINGLE) : this.current));
        if (opcode == 167) {
            this.current.clear();
        }
    ...
    public void visitTableSwitchInsn(int minimum, int maximum, Label defaultOption, Label... option) {
        ...
        visitTableSwitchInsn(minimum, maximum, defaultOption, option);
    }
    ...
    public void visitLookupSwitchInsn(Label defaultOption, int[] key, Label[] option) {
        ...
        visitLookupSwitchInsn(defaultOption, key, option);
    }
