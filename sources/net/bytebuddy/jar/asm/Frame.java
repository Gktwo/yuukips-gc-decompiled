...
/* loaded from: grasscutter.jar:net/bytebuddy/jar/asm/Frame.class */
public class Frame {
    ...
    public static int getAbstractTypeFromApiFormat(SymbolTable symbolTable, Object type) {
        ...
        if (type instanceof String) {
            return getAbstractTypeFromDescriptor(symbolTable, Type.getObjectType((String) type).getDescriptor(), 0);
        }
    ...
    private static int getAbstractTypeFromDescriptor(SymbolTable symbolTable, String buffer, int offset) {
        ...
        switch (buffer.charAt(offset)) {
            ...
            case 'J':
                return LONG;
            case 'L':
                return 8388608 | symbolTable.addType(buffer.substring(offset + 1, buffer.length() - 1));
            case 'V':
                return 0;
            case '[':
                ...
                switch (buffer.charAt(elementDescriptorOffset)) {
                    ...
                    case 'J':
                        ...
                        break;
                    case 'L':
                        typeValue = 8388608 | symbolTable.addType(buffer.substring(elementDescriptorOffset + 1, buffer.length() - 1));
                        break;
                    case 'S':
                        typeValue = SHORT;
                        break;
    ...
    public final void setInputFrameFromApiFormat(SymbolTable symbolTable, int numLocal, Object[] local, int numStack, Object[] stack) {
        ...
        for (int i3 = 0; i3 < numStack; i3++) {
            inputStackIndex++;
            this.inputStack[inputStackIndex] = getAbstractTypeFromApiFormat(symbolTable, stack[i3]);
            if (stack[i3] == Opcodes.LONG || stack[i3] == Opcodes.DOUBLE) {
                inputStackIndex++;
    ...
    private void push(SymbolTable symbolTable, String descriptor) {
        int abstractType = getAbstractTypeFromDescriptor(symbolTable, descriptor, descriptor.charAt(0) == '(' ? Type.getReturnTypeOffset(descriptor) : 0);
        if (abstractType != 0) {
            push(abstractType);
            if (abstractType == LONG || abstractType == DOUBLE) {
                push(4194304);
            ...
            case 185:
                ...
                if (opcode != 184) {
                    ...
                    if (opcode == 183 && argSymbol.name.charAt(0) == '<') {
                        addInitializedType(abstractType19);
                    }
                }
                push(symbolTable, argSymbol.value);
                return;
    ...
    private static boolean merge(SymbolTable symbolTable, int sourceType, int[] dstTypes, int dstIndex) {
        ...
        if ((sourceType & Lua.MAXARG_Ax) == NULL) {
            if (dstType == NULL) {
                return false;
            }
            srcType = NULL;
        ...
        if ((dstType & -67108864) != 0 || (dstType & KIND_MASK) == 8388608) {
            ...
            if ((srcType & -4194304) == (dstType & -4194304)) {
                if ((dstType & KIND_MASK) == 8388608) {
                    ...
                    mergedType = (-67108864 + (srcType & -67108864)) | 8388608 | symbolTable.addType(TypeProxy.SilentConstruction.Appender.JAVA_LANG_OBJECT_INTERNAL_NAME);
                }
    ...
    public final void accept(MethodWriter methodWriter) {
        ...
        while (true) {
            numStack--;
            if (numStack > 0) {
                int stackType2 = stackTypes[i4];
                i4 += (stackType2 == LONG || stackType2 == DOUBLE) ? 2 : 1;
                frameIndex++;
