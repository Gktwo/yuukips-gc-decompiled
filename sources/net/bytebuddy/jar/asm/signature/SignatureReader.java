...
/* loaded from: grasscutter.jar:net/bytebuddy/jar/asm/signature/SignatureReader.class */
public class SignatureReader {
    ...
    public void accept(SignatureVisitor signatureVistor) {
        ...
        if (signature.charAt(offset) == '(') {
            ...
            int offset4 = parseType(signature, offset3 + 1, signatureVistor.visitReturnType());
            while (offset4 < length) {
                offset4 = parseType(signature, offset4 + 1, signatureVistor.visitExceptionType());
            }
            return;
        }
        int offset5 = parseType(signature, offset, signatureVistor.visitSuperclass());
        while (offset5 < length) {
            offset5 = parseType(signature, offset5, signatureVistor.visitInterface());
        }
    }
