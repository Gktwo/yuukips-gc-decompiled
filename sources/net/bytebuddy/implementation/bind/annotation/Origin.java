...
/* loaded from: grasscutter.jar:net/bytebuddy/implementation/bind/annotation/Origin.class */
public @interface Origin {
    boolean cache() default true;

    boolean privileged() default false;

    /* loaded from: grasscutter.jar:net/bytebuddy/implementation/bind/annotation/Origin$Binder.class */
    public enum Binder implements TargetMethodAnnotationDrivenBinder.ParameterBinder<Origin> {
        ...
        private static StackManipulation methodConstant(Origin origin, MethodDescription.InDefinedShape methodDescription) {
            ...
            if (origin.privileged()) {
                methodConstant = MethodConstant.ofPrivileged(methodDescription);
            } else {
                methodConstant = MethodConstant.m112of(methodDescription);
            }
        ...
        public MethodDelegationBinder.ParameterBinding<?> bind(AnnotationDescription.Loadable<Origin> annotation, MethodDescription source, ParameterDescription target, Implementation.Target implementationTarget, Assigner assigner, Assigner.Typing typing) {
            ...
            if (parameterType.represents(Class.class)) {
                return new MethodDelegationBinder.ParameterBinding.Anonymous(ClassConstant.m114of(implementationTarget.getOriginType().asErasure()));
            }
            ...
            if (parameterType.represents(Constructor.class)) {
            ...
            if (parameterType.equals(JavaType.METHOD_HANDLE.getTypeStub())) {
                return new MethodDelegationBinder.ParameterBinding.Anonymous(new JavaConstantValue(JavaConstant.MethodHandle.m76of(source.asDefined())));
            }
            if (parameterType.equals(JavaType.METHOD_TYPE.getTypeStub())) {
                return new MethodDelegationBinder.ParameterBinding.Anonymous(new JavaConstantValue(JavaConstant.MethodType.m70of(source.asDefined())));
            }
