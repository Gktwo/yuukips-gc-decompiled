...
/* loaded from: grasscutter.jar:ch/qos/logback/core/rolling/FixedWindowRollingPolicy.class */
public class FixedWindowRollingPolicy extends RollingPolicyBase {
    ...
    private String transformFileNamePatternFromInt2Date(String fileNamePatternStr) {
        return FileFilterUtil.afterLastSlash(FileFilterUtil.slashify(fileNamePatternStr)).replace("%i", "%d{yyyy-MM-dd_HHmm}");
    }
    ...
    public void rollover() throws RolloverFailure {
        if (this.maxIndex >= 0) {
            ...
            for (int i = this.maxIndex - 1; i >= this.minIndex; i--) {
                ...
                if (new File(toRenameStr).exists()) {
