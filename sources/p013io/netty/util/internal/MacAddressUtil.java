...
/* loaded from: grasscutter.jar:io/netty/util/internal/MacAddressUtil.class */
public final class MacAddressUtil {
    ...
    public static String formatAddress(byte[] addr) {
        ...
        for (int i = 0; i < length; i++) {
            buf.append(String.format("%02x:", Integer.valueOf(addr[i] & 255)));
        }
    ...
    static int compareAddresses(byte[] current, byte[] candidate) {
        ...
        if ((candidate[0] & 2) == 0) {
            ...
            return 0;
        } else if (current.length == 0 || (current[0] & 2) != 0) {
            return 0;
        } else {
            return 1;
        }
