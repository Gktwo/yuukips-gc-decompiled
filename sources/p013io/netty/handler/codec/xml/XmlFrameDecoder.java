package p013io.netty.handler.codec.xml;
...
/* loaded from: grasscutter.jar:io/netty/handler/codec/xml/XmlFrameDecoder.class */
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        ...
        for (int i = in.readerIndex(); i < bufferLength; i++) {
            ...
            if (!openingBracketFound && Character.isWhitespace(readByte)) {
                ...
                in.skipBytes(in.readableBytes());
                return;
            } else if (!inCDATASection && readByte == 60) {
                    ...
                    if (peekAheadByte == 47) {
                        ...
                        while (true) {
                            if (peekFurtherAheadIndex > bufferLength - 1) {
                                break;
                            } else if (in.getByte(peekFurtherAheadIndex) == 62) {
                                c--;
                                break;
                            } else {
                ...
                if (readByte == 62) {
                    ...
                    if (atLeastOneXmlElementFound && c == 0) {
                        break;
                    }
                } else {
                    continue;
                }
            } else if (i < bufferLength - 1 && in.getByte(i + 1) == 62) {
                c--;
            }
        }
        int readerIndex = in.readerIndex();
        int xmlElementLength = length - readerIndex;
        if (c == 0 && xmlElementLength > 0) {
