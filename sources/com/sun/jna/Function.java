...
/* loaded from: grasscutter.jar:com/sun/jna/Function.class */
public class Function extends Pointer {
    ...
    /* loaded from: grasscutter.jar:com/sun/jna/Function$PostCallRead.class */
    ...
    public Function(NativeLibrary library, String functionName, int callFlags, String encoding) {
        checkCallingConvention(callFlags & 63);
    ...
    public Object invoke(Class<?> returnType, Object[] inArgs, Map<String, ?> options) {
        ...
        return invoke(invokingMethod, invokingMethod != null ? invokingMethod.getParameterTypes() : null, returnType, inArgs, options);
    }
    ...
    public Object invoke(Method invokingMethod, Class<?>[] paramTypes, Class<?> returnType, Object[] inArgs, Map<String, ?> options) {
        ...
        boolean isVarArgs = (args.length <= 0 || invokingMethod == null) ? false : isVarArgs(invokingMethod);
        int fixedArgs = (args.length <= 0 || invokingMethod == null) ? 0 : fixedArgs(invokingMethod);
        int i = 0;
        while (i < args.length) {
            if (invokingMethod != null) {
                paramType = (!isVarArgs || i < paramTypes.length - 1) ? paramTypes[i] : paramTypes[paramTypes.length - 1].getComponentType();
            } else {
                paramType = null;
            }
                    ...
                    if (inArg instanceof Structure) {
                        ...
                        if (args[i2] instanceof PointerArray) {
                            ...
                            if (Structure.ByReference[].class.isAssignableFrom(inArg.getClass())) {
                                Class<?> componentType = inArg.getClass().getComponentType();
                                Structure[] ss = (Structure[]) inArg;
                                for (int si = 0; si < ss.length; si++) {
                                    ss[si] = Structure.updateStructureByReference(componentType, ss[si], array.getPointer((long) (Native.POINTER_SIZE * si)));
                                }
                            }
                        }
        ...
        if (returnType == null || returnType == Void.TYPE || returnType == Void.class) {
            ...
            if (Structure.class.isAssignableFrom(returnType)) {
                if (Structure.ByValue.class.isAssignableFrom(returnType)) {
                    Structure s2 = Native.invokeStructure(this, this.peer, callFlags, args, Structure.newInstance(returnType));
                    s2.autoRead();
                    result = s2;
                } else {
                    result = invokePointer(callFlags, args);
                    if (result != null) {
                        Structure s3 = Structure.newInstance((Class<Structure>) returnType, (Pointer) result);
                ...
                if (p != null) {
                    result = p.getStringArray(0, this.encoding);
                }
            } else if (returnType == WString[].class) {
                Pointer p2 = invokePointer(callFlags, args);
                if (p2 != null) {
                    String[] arr = p2.getWideStringArray(0);
                ...
                Pointer p3 = invokePointer(callFlags, args);
                if (p3 != null) {
                    result = p3.getPointerArray(0);
                }
    ...
    private Object convertArgument(Object[] args, int index, Method invokingMethod, TypeMapper mapper, boolean allowObjects, Class<?> expectedType) {
        ...
        if (arg instanceof Structure) {
            ...
            if (Structure[].class.isAssignableFrom(argClass)) {
                ...
                boolean byRef = Structure.ByReference.class.isAssignableFrom(type2);
                if (expectedType != null && !Structure.ByReference[].class.isAssignableFrom(expectedType)) {
                    if (byRef) {
                        throw new IllegalArgumentException("Function " + getName() + " declared Structure[] at parameter " + index + " but array of " + type2 + " was passed");
                    }
    ...
    private String invokeString(int callFlags, Object[] args, boolean wide) {
        ...
        if (ptr != null) {
            if (wide) {
                ...
                s = ptr.getString(0, this.encoding);
            }
        }
        return s;
    }
    ...
    @Override // com.sun.jna.Pointer
    public int hashCode() {
        return this.callFlags + this.options.hashCode() + hashCode();
    }
            ...
            if (argType != null && argType.isArray()) {
                ...
                for (int i = 0; i < varArgs.length; i++) {
                    if (varArgs[i] instanceof Float) {
                        varArgs[i] = Double.valueOf((double) ((Float) varArgs[i]).floatValue());
                    }
                }
    ...
    /* loaded from: grasscutter.jar:com/sun/jna/Function$NativeMappedArray.class */
    public static class NativeMappedArray extends Memory implements PostCallRead {
        ...
        public NativeMappedArray(NativeMapped[] arg) {
            ...
            this.original = arg;
            setValue(0, this.original, this.original.getClass());
        }
    ...
    /* access modifiers changed from: private */
    /* loaded from: grasscutter.jar:com/sun/jna/Function$PointerArray.class */
    public static class PointerArray extends Memory implements PostCallRead {
        ...
        @Override // com.sun.jna.Function.PostCallRead
        public void read() {
            read(0, this.original, 0, this.original.length);
        }
    }
