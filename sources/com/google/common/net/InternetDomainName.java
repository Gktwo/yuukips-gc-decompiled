...
@Beta
@GwtCompatible
/* loaded from: grasscutter.jar:com/google/common/net/InternetDomainName.class */
public final class InternetDomainName {
    ...
    private static final Splitter DOT_SPLITTER = Splitter.m1455on('.');
    private static final Joiner DOT_JOINER = Joiner.m1465on('.');
    private static final CharMatcher DASH_MATCHER = CharMatcher.anyOf("-_");
    private static final CharMatcher PART_CHAR_MATCHER = CharMatcher.javaLetterOrDigit().mo1466or(DASH_MATCHER);

    InternetDomainName(String name) {
        String name2 = Ascii.toLowerCase(DOTS_MATCHER.replaceFrom((CharSequence) name, '.'));
        name2 = name2.endsWith(Mapper.IGNORED_FIELDNAME) ? name2.substring(0, name2.length() - 1) : name2;
        ...
        this.registrySuffixIndex = findSuffixOfType(Optional.m1463of(PublicSuffixType.REGISTRY));
    }
    ...
    public InternetDomainName child(String leftParts) {
        return from(((String) Preconditions.checkNotNull(leftParts)) + Mapper.IGNORED_FIELDNAME + this.name);
    }
