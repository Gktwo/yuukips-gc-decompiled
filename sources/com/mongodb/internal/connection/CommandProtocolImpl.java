...
/* loaded from: grasscutter.jar:com/mongodb/internal/connection/CommandProtocolImpl.class */
class CommandProtocolImpl<T> implements CommandProtocol<T> {
    ...
    public CommandProtocolImpl(String database, BsonDocument command, FieldNameValidator commandFieldNameValidator, ReadPreference readPreference, Decoder<T> commandResultDecoder, boolean responseExpected, SplittablePayload payload, FieldNameValidator payloadFieldNameValidator, ClusterConnectionMode clusterConnectionMode) {
    ...
    public T execute(InternalConnection connection) {
        return (T) connection.sendAndReceive(getCommandMessage(connection), this.commandResultDecoder, this.sessionContext);
    }
