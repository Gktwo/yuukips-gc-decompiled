...
/* loaded from: grasscutter.jar:com/mongodb/client/gridfs/GridFSBucketImpl.class */
final class GridFSBucketImpl implements GridFSBucket {
    ...
    public GridFSBucketImpl(MongoDatabase database) {
        this(database, GridFS.DEFAULT_BUCKET);
    }

    /* access modifiers changed from: package-private */
    public GridFSBucketImpl(MongoDatabase database, String bucketName) {
        this((String) Assertions.notNull("bucketName", bucketName), 261120, getFilesCollection((MongoDatabase) Assertions.notNull("database", database), bucketName), getChunksCollection(database, bucketName));
    }
    ...
    private void executeUploadFromStream(@Nullable ClientSession clientSession, BsonValue id, String filename, InputStream source, GridFSUploadOptions options) {
        ...
        Integer chunkSizeBytes = options.getChunkSizeBytes();
        byte[] buffer = new byte[chunkSizeBytes == null ? this.chunkSizeBytes : chunkSizeBytes.intValue()];
        while (true) {
            try {
    ...
    private void executeDelete(@Nullable ClientSession clientSession, BsonValue id) {
        ...
        if (clientSession != null) {
            result = this.filesCollection.deleteOne(clientSession, new BsonDocument(DBCollection.ID_FIELD_NAME, id));
            this.chunksCollection.deleteMany(clientSession, new BsonDocument("files_id", id));
        } else {
            result = this.filesCollection.deleteOne(new BsonDocument(DBCollection.ID_FIELD_NAME, id));
    ...
    private void executeRename(@Nullable ClientSession clientSession, BsonValue id, String newFilename) {
        ...
        if (clientSession != null) {
            updateResult = this.filesCollection.updateOne(clientSession, new BsonDocument(DBCollection.ID_FIELD_NAME, id), new BsonDocument("$set", new BsonDocument("filename", new BsonString(newFilename))));
        } else {
            updateResult = this.filesCollection.updateOne(new BsonDocument(DBCollection.ID_FIELD_NAME, id), new BsonDocument("$set", new BsonDocument("filename", new BsonString(newFilename))));
        }
    ...
    private <T> boolean collectionIsEmpty(@Nullable ClientSession clientSession, MongoCollection<T> collection) {
        return clientSession != null ? collection.find(clientSession).projection(new Document(DBCollection.ID_FIELD_NAME, 1)).first() == null : collection.find().projection(new Document(DBCollection.ID_FIELD_NAME, 1)).first() == null;
    }
    ...
    private <T> boolean hasIndex(@Nullable ClientSession clientSession, MongoCollection<T> collection, Document index) {
        ...
        Iterator<Document> it = ((ArrayList) listIndexesIterable.into(new ArrayList())).iterator();
        while (true) {
            if (!it.hasNext()) {
                break;
            }
            Document indexDoc = (Document) it.next().get((Object) Action.KEY_ATTRIBUTE, (String) new Document());
    ...
    private GridFSFile getFileInfoById(@Nullable ClientSession clientSession, BsonValue id) {
        ...
        GridFSFile fileInfo = createFindIterable(clientSession, new Document(DBCollection.ID_FIELD_NAME, id)).first();
        if (fileInfo != null) {
            return fileInfo;
        }
        throw new MongoGridFSException(String.format("No file found with the id: %s", id));
