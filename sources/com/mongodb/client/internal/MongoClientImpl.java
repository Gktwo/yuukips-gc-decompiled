...
/* loaded from: grasscutter.jar:com/mongodb/client/internal/MongoClientImpl.class */
public final class MongoClientImpl implements MongoClient {
    ...
    public MongoClientImpl(Cluster cluster, MongoDriverInformation mongoDriverInformation, MongoClientSettings settings, @Nullable OperationExecutor operationExecutor) {
        ...
        this.delegate = new MongoClientDelegate((Cluster) Assertions.notNull("cluster", cluster), CodecRegistryHelper.createRegistry(settings.getCodecRegistry(), settings.getUuidRepresentation()), this, operationExecutor, autoEncryptionSettings == null ? null : Crypts.createCrypt(this, autoEncryptionSettings));
    }
    ...
    private <TResult> ChangeStreamIterable<TResult> createChangeStreamIterable(@Nullable ClientSession clientSession, List<? extends Bson> pipeline, Class<TResult> resultClass) {
        return new ChangeStreamIterableImpl(clientSession, "admin", this.settings.getCodecRegistry(), this.settings.getReadPreference(), this.settings.getReadConcern(), this.delegate.getOperationExecutor(), pipeline, resultClass, ChangeStreamLevel.CLIENT, this.settings.getRetryReads());
    }
    ...
    /* JADX DEBUG: Type inference failed for r0v3. Raw type applied. Possible types: com.mongodb.client.MongoIterable<U>, com.mongodb.client.MongoIterable<java.lang.String> */
