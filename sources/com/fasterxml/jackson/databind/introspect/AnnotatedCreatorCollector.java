...
/* loaded from: grasscutter.jar:com/fasterxml/jackson/databind/introspect/AnnotatedCreatorCollector.class */
public final class AnnotatedCreatorCollector extends CollectorBase {
    ...
    private List<AnnotatedConstructor> _findPotentialConstructors(JavaType type, Class<?> primaryMixIn) {
        ...
        if (primaryMixIn != null) {
            ...
            ClassUtil.Ctor[] constructors = ClassUtil.getConstructors(primaryMixIn);
            for (ClassUtil.Ctor mixinCtor : constructors) {
    ...
    private List<AnnotatedMethod> _findPotentialFactories(TypeFactory typeFactory, JavaType type, Class<?> primaryMixIn) {
        ...
        Method[] classMethods = ClassUtil.getClassMethods(type.getRawClass());
        ...
        if (primaryMixIn != null) {
            ...
            Method[] declaredMethods = primaryMixIn.getDeclaredMethods();
            for (Method mixinFactory : declaredMethods) {
        ...
        for (int i4 = 0; i4 < factoryCount; i4++) {
            if (result.get(i4) == null) {
                ...
                result.set(i4, constructFactoryCreator(candidate, MethodGenericTypeResolver.narrowMethodTypeParameters(candidate, type, typeFactory, initialTypeResCtxt), null));
            }
        }
        return result;
    }
    ...
    protected AnnotatedMethod constructFactoryCreator(Method m, TypeResolutionContext typeResCtxt, Method mixin) {
        ...
        return new AnnotatedMethod(typeResCtxt, m, collectAnnotations(m, mixin), collectAnnotations(m.getParameterAnnotations(), mixin == null ? null : mixin.getParameterAnnotations()));
    }
