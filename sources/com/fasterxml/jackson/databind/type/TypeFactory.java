...
/* loaded from: grasscutter.jar:com/fasterxml/jackson/databind/type/TypeFactory.class */
public class TypeFactory implements Serializable {
        ...
        if (rawBase == Object.class) {
            ...
            if (baseType.getBindings().isEmpty()) {
                ...
                if (typeParamCount == 0) {
                    ...
                    newType = _fromClass(null, subclass, _bindingsForSubtype(baseType, typeParamCount, subclass, relaxedCompatibilityCheck));
                }
            }
        }
    ...
    private TypeBindings _bindingsForSubtype(JavaType baseType, int typeParamCount, Class<?> subclass, boolean relaxedCompatibilityCheck) {
        ...
        JavaType baseWithPlaceholders = _fromClass(null, subclass, TypeBindings.create(subclass, placeholders)).findSuperType(baseType.getRawClass());
    ...
    private boolean _verifyAndResolvePlaceholders(JavaType exp, JavaType act) {
        if (act instanceof PlaceholderForType) {
            ...
            for (int i = 0; i < len; i++) {
                if (!_verifyAndResolvePlaceholders(expectedTypes.get(i), actualTypes.get(i))) {
                    return false;
                }
            }
            return true;
    ...
    public JavaType constructType(Type type, TypeBindings bindings) {
        if (type instanceof Class) {
            return _applyModifiers(type, _fromClass(null, (Class) type, bindings));
        }
    ...
    public JavaType constructParametricType(Class<?> rawType, TypeBindings parameterTypes) {
        return _applyModifiers(rawType, _fromClass(null, rawType, parameterTypes));
    }
    ...
    protected JavaType _applyModifiers(Type srcType, JavaType resolvedType) {
        ...
        for (TypeModifier mod : typeModifierArr) {
