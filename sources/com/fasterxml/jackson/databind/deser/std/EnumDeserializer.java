...
@JacksonStdImpl
/* loaded from: grasscutter.jar:com/fasterxml/jackson/databind/deser/std/EnumDeserializer.class */
public class EnumDeserializer extends StdScalarDeserializer<Object> implements ContextualDeserializer {
    ...
    private final Object _deserializeAltString(JsonParser p, DeserializationContext ctxt, CompactStringObjectMap lookup, String nameOrig) throws IOException {
        ...
        if (!name.isEmpty()) {
            ...
            if (nameOrig.isEmpty()) {
                act = _checkCoercionFail(ctxt, _findCoercionFromEmptyString(ctxt), handledType(), nameOrig, "empty String (\"\")");
            } else {
                act = _checkCoercionFail(ctxt, _findCoercionFromBlankString(ctxt), handledType(), nameOrig, "blank String (all whitespace)");
            }
            switch (act) {
                case AsNull:
                default:
