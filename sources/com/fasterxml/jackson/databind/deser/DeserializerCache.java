...
/* loaded from: grasscutter.jar:com/fasterxml/jackson/databind/deser/DeserializerCache.class */
public final class DeserializerCache implements Serializable {
    ...
    public DeserializerCache(int maxSize) {
        ...
        this._cachedDeserializers = new LRUMap<>(Math.min(64, maxSize >> 2), maxSize);
    }
    ...
    protected JsonDeserializer<?> _createDeserializer2(DeserializationContext ctxt, DeserializerFactory factory, JavaType type, BeanDescription beanDesc) throws JsonMappingException {
        ...
        if (type.isContainerType()) {
            ...
            if (type.isMapLikeType() && beanDesc.findExpectedFormat(null).getShape() != JsonFormat.Shape.OBJECT) {
                MapLikeType mlt = (MapLikeType) type;
                if (mlt instanceof MapType) {
                    return factory.createMapDeserializer(ctxt, (MapType) mlt, beanDesc);
                }
                return factory.createMapLikeDeserializer(ctxt, mlt, beanDesc);
            } else if (type.isCollectionLikeType() && beanDesc.findExpectedFormat(null).getShape() != JsonFormat.Shape.OBJECT) {
