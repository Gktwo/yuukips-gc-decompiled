...
/* loaded from: grasscutter.jar:com/fasterxml/jackson/databind/jsontype/impl/AsDeductionTypeDeserializer.class */
public class AsDeductionTypeDeserializer extends AsPropertyTypeDeserializer {
    ...
    protected Map<BitSet, String> buildFingerprints(DeserializationConfig config, Collection<NamedType> subtypes) {
        ...
        for (NamedType subtype : subtypes) {
            List<BeanPropertyDefinition> properties = config.introspect(config.getTypeFactory().constructType(subtype.getType())).findProperties();
    ...
    public Object deserializeTypedFromObject(JsonParser p, DeserializationContext ctxt) throws IOException {
        ...
        return _deserializeTypedUsingDefaultImpl(p, ctxt, tb, String.format("Cannot deduce unique subtype of %s (%d candidates match)", ClassUtil.getTypeDescription(this._baseType), Integer.valueOf(candidates.size())));
    }
