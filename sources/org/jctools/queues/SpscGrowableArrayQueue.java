...
/* loaded from: grasscutter.jar:org/jctools/queues/SpscGrowableArrayQueue.class */
public class SpscGrowableArrayQueue<E> extends BaseSpscLinkedArrayQueue<E> {
    ...
    /* JADX DEBUG: Multi-variable search result rejected for r11v0, resolved type: org.jctools.queues.SpscGrowableArrayQueue<E> */
    /* JADX WARN: Multi-variable type inference failed */
    ...
    final boolean offerColdPath(E[] buffer, long mask, long index, long offset, E v, MessagePassingQueue.Supplier<? extends E> s) {
        ...
        if (lookAheadStep <= 0) {
            ...
            return true;
        } else if (null == UnsafeRefArrayAccess.lvRefElement(buffer, UnsafeRefArrayAccess.calcCircularRefElementOffset(index + lookAheadStep, mask))) {
            ...
            if (mask + 1 == ((long) maxCapacity2)) {
                ...
                writeToQueue(buffer, v == null ? s.get() : v, index, offset);
                return true;
            } else if (null == UnsafeRefArrayAccess.lvRefElement(buffer, UnsafeRefArrayAccess.calcCircularRefElementOffset(index + 1, mask))) {
                writeToQueue(buffer, v == null ? s.get() : v, index, offset);
                return true;
            } else {
                Object[] allocateRefArray = UnsafeRefArrayAccess.allocateRefArray((int) ((2 * (mask + 1)) + 1));
                this.producerBuffer = allocateRefArray;
                this.producerMask = (long) (LinkedArrayQueueUtil.length(allocateRefArray) - 2);
                linkOldToNew(index, buffer, offset, allocateRefArray, UnsafeRefArrayAccess.calcCircularRefElementOffset(index, this.producerMask), v == null ? s.get() : v);
    ...
    private void adjustLookAheadStep(int capacity) {
        this.lookAheadStep = (long) Math.min(capacity / 4, SpscArrayQueue.MAX_LOOK_AHEAD_STEP);
    }
