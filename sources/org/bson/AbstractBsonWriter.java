...
/* loaded from: grasscutter.jar:org/bson/AbstractBsonWriter.class */
public abstract class AbstractBsonWriter implements BsonWriter, Closeable {
    ...
    /* loaded from: grasscutter.jar:org/bson/AbstractBsonWriter$State.class */
    public enum State {
        INITIAL,
        NAME,
        VALUE,
    ...
    public AbstractBsonWriter(BsonWriterSettings settings) {
        this(settings, new NoOpFieldNameValidator());
    }

    /* access modifiers changed from: protected */
    public AbstractBsonWriter(BsonWriterSettings settings, FieldNameValidator validator) {
    ...
    public void writeStartDocument() {
        ...
        if (!(this.context == null || this.context.name == null)) {
            FieldNameValidator validator = this.fieldNameValidatorStack.peek().getValidatorForField(getName());
            this.fieldNameValidatorStack.push(validator);
            validator.start();
        }
    ...
    protected void checkPreconditions(String methodName, State... validStates) {
        ...
        } else if (!checkState(validStates)) {
    ...
    protected void throwInvalidContextType(String methodName, BsonContextType actualContextType, BsonContextType... validContextTypes) {
        throw new BsonInvalidOperationException(String.format("%s can only be called when ContextType is %s, not when ContextType is %s.", methodName, StringUtils.join(" or ", Arrays.asList(validContextTypes)), actualContextType));
    }
    ...
    protected void throwInvalidState(String methodName, State... validStates) {
        ...
        throw new BsonInvalidOperationException(String.format("%s can only be called when State is %s, not when State is %s", methodName, StringUtils.join(" or ", Arrays.asList(validStates)), this.state));
    }
    ...
    /* loaded from: grasscutter.jar:org/bson/AbstractBsonWriter$Context.class */
    ...
    /* access modifiers changed from: protected */
    /* loaded from: grasscutter.jar:org/bson/AbstractBsonWriter$Mark.class */
    public class Mark {
