...
/* loaded from: grasscutter.jar:org/bson/internal/Base64.class */
public final class Base64 {
    ...
    public static byte[] decode(String s) {
        byte[] buffer = new byte[((s.length() * 3) / 4) - (s.endsWith("==") ? 2 : s.endsWith("=") ? 1 : 0)];
        int pos = 0;
        for (int i = 0; i < s.length(); i += 4) {
            ...
            pos = pos3 + 1;
            buffer[pos3] = (byte) (((c2 << 6) | DECODE_TABLE[s.charAt(i + 3)]) & 255);
        }
        return buffer;
    }
    ...
    public static String encode(byte[] in) {
        int modulus = 0;
        int bitWorkArea = 0;
        byte[] buffer = new byte[((in.length / 3) * 4) + (in.length % 3 == 0 ? 0 : 4)];
        int pos = 0;
        for (int b : in) {
            modulus = (modulus + 1) % 3;
            if (b < 0) {
